	...
	Partial Public Class Form1
		...
		Private Sub spreadsheetControl1_CustomDrawCell(ByVal sender As Object, ByVal e As CustomDrawCellEventArgs) Handles spreadsheetControl1.CustomDrawCell
			...
			If e.Cell.ColumnIndex = 4 AndAlso e.Cell.Value.IsNumeric AndAlso e.Cell.Value.NumericValue = 0 Then
				...
				Using font As New Font(e.Font.Name, 9F, FontStyle.Bold)
					Using format As New StringFormat()
						...
						Dim textBounds As New Rectangle(e.Bounds.Right + 15, CInt(Math.Truncate(Math.Round(e.Bounds.Top + (e.Bounds.Height - size.Height) \ 2))), CInt(size.Width + 8), CInt(Math.Round(size.Height)))
		...
		Private Function ChooseImage(ByVal categoryName As String) As Image
		...
		Private Sub spreadsheetControl1_CustomDrawCellBackground(ByVal sender As Object, ByVal e As CustomDrawCellBackgroundEventArgs) Handles spreadsheetControl1.CustomDrawCellBackground
			' Specify the background color for the active cell. 
			If e.Cell = spreadsheetControl1.ActiveCell Then
		...
		Private Sub spreadsheetControl1_CustomDrawColumnHeader(ByVal sender As Object, ByVal e As CustomDrawColumnHeaderEventArgs) Handles spreadsheetControl1.CustomDrawColumnHeader
			...
			Dim headers() As String = { "Category", "Product Name", "Quantity per Unit", "Unit Price", "Units in Stock" }
		...
		Private Sub spreadsheetControl1_CustomDrawColumnHeaderBackground(ByVal sender As Object, ByVal e As CustomDrawColumnHeaderBackgroundEventArgs) Handles spreadsheetControl1.CustomDrawColumnHeaderBackground
			...
			If e.ColumnIndex <= 4 Then
				Dim backColor = If(e.IsHovered OrElse (e.ColumnIndex = spreadsheetControl1.ActiveCell.ColumnIndex), Color.FromArgb(200, 44, 74), Color.FromArgb(91, 155, 213))
		...
		Private Sub spreadsheetControl1_CustomDrawRowHeader(ByVal sender As Object, ByVal e As CustomDrawRowHeaderEventArgs) Handles spreadsheetControl1.CustomDrawRowHeader
			...
			If (e.RowIndex + 1) Mod 5 = 0 Then
				...
				e.Graphics.DrawString(text, e.Font, e.Cache.GetSolidBrush(Color.FromArgb(91, 155, 213)), textBounds, format)
			End If
		End Sub
		#End Region ' CustomDrawRowHeader

		#Region "CustomDrawRowHeaderBackground"
		Private Sub spreadsheetControl1_CustomDrawRowHeaderBackground(ByVal sender As Object, ByVal e As CustomDrawRowHeaderBackgroundEventArgs) Handles spreadsheetControl1.CustomDrawRowHeaderBackground
			...
			Dim backColor As Color = If(e.IsHovered OrElse (e.RowIndex = spreadsheetControl1.ActiveCell.RowIndex), Color.FromArgb(200, 44, 74), Color.White)
			e.Cache.FillRectangle(e.Cache.GetSolidBrush(backColor), e.Bounds)
		End Sub
